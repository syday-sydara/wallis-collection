...
export default function CartSummary({ items, shippingCost = 0, onQuantityChange, onRemove }: Props) {
  ...
  return (
    <div className="bg-surface-muted p-4 rounded-lg shadow-sm space-y-4 animate-fadeIn-fast pb-safe">
      ...
      {items.length === 0 && <p className="text-sm text-text-muted">Your cart is empty.</p>}
        ...
        {items.map(item => (
          <li key={item.id} className="flex gap-3 items-center leading-none">
            ...
            <div className="flex-1 min-w-0">
              ...
              {item.attributes && <p className="text-xs text-text-muted truncate leading-none">{Object.values(item.attributes).join(" / ")}</p>}
              <p className="text-sm text-text mt-1 leading-none">{formatCurrency(item.unitPrice)}</p>
            </div>
            {onQuantityChange && (
              <div className="flex items-center gap-1">
                <button type="button" onClick={() => onQuantityChange(item.id, Math.max(1, item.quantity - 1))} disabled={item.quantity <= 1} className="px-2 py-1 border rounded-md text-sm disabled:opacity-50 min-h-touch active:scale-press transition">−</button>
                <span className="w-6 text-center text-sm leading-none">{item.quantity}</span>
                <button type="button" onClick={() => onQuantityChange(item.id, item.quantity + 1)} className="px-2 py-1 border rounded-md text-sm min-h-touch active:scale-press transition">+</button>
              </div>
            )}
            {onRemove && <button type="button" onClick={() => onRemove(item.id)} className="ml-2 text-danger text-sm hover:underline active:scale-press transition">Remove</button>}
          </li>
        ))}
      </ul>
