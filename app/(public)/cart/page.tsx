      ...
      {!isEmpty && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            {items.map((item) => (
              <div key={item.key} className="flex items-center gap-4 p-4 border border-[var(--color-border)] rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-[var(--color-text-secondary)]">₦{item.price.toLocaleString()}</p>

                  <div className="flex items-center mt-2 space-x-3">
                    <button onClick={() => decrement(item.key)} className="px-2 py-1 border rounded hover:bg-[var(--color-bg-surface)] transition">-</button>
                    <span className="font-medium">{item.quantity}</span>
                    <button onClick={() => increment(item.key)} className="px-2 py-1 border rounded hover:bg-[var(--color-bg-surface)] transition">+</button>
                  </div>
                </div>
                <button onClick={() => removeItem(item.key)} className="text-[var(--color-danger-500)] hover:opacity-70">
                  <X size={20} />
                </button>
              </div>
            ))}
          ...
          <div className="p-6 border border-[var(--color-border)] rounded-lg h-fit space-y-6">
            ...
            <Button variant="subtle" className="w-full" onClick={clearCart}>Clear Cart</Button>
          </div>
        </div>
      )}
    </div>
